...
public class ProtocolMessageSerializerTest {
    ...
    void serializeAndDeserialize_BinaryPayload_ReturnsOriginalMessage() throws IOException {
        ...
        ProtocolMessage<byte[]> originalMessage =
                new ProtocolMessage<>(timestamp, Action.CREATE_LOBBY, originalPayload);

        byte[] serialized = serializer.serialize(originalMessage);
        @SuppressWarnings("unchecked")
        ProtocolMessage<byte[]> deserialized = (ProtocolMessage<byte[]>) serializer.deserialize(serialized);
    ...
    void serialize_ProducesExpectedFormat() throws JsonProcessingException {
        ...
        ProtocolMessage<byte[]> message = new ProtocolMessage<>(timestamp, Action.CREATE_LOBBY, payload);
        ...
        */
        byte[] expectedFormat = new byte[] {
            0, 0, 1, -108, 33, -68, -81, -36, 12, 67, 82, 69, 65, 84, 69, 95, 76, 79, 66, 66, 89, 0, 0, 0, 0, 3, 0, 1, 2
        };
